...
const locationPolicyHtml = computed(() => marked.parse(LOCATION_POLICY_TEXT, { breaks: true }));
const serviceTermsHtml = computed(() => marked.parse(SERVICE_TERMS_TEXT, { breaks: true }));
...
const activeModal = computed(() => {
  ...
  const map = {
    ...
    locationPolicy: { title: '위치정보 이용약관', html: locationPolicyHtml.value }
  };
